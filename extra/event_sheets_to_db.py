...
def calculate_data_hash(data):
    """Calculate a hash for the spreadsheet data for comparison purposes."""
    hash_md5 = hashlib.md5()
    for row in data:
        row_string = ''.join(map(str, row))  # Convert the row to a string
        hash_md5.update(row_string.encode('utf-8'))
    return hash_md5.hexdigest()

def poll_and_update_event_based():
    ...
    while True:
        ...
        if new_data_hash != last_data_hash:
            ...
            if rows_to_insert_or_update:
                ...
                insert_into_mysql([['ID', 'Company Name', 'Job Title', 'CGPA Cut-off', 'Remarks']] + rows_to_insert_or_update)

            if ids_to_delete:
                print("Deleting rows...")
                delete_from_mysql(ids_to_delete)

            # Update last_data and last_data_hash after processing
            last_data = new_data
