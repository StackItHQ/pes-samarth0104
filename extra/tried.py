...
def db_to_sheets_sync(last_sheet_hash):
    """Synchronize data from MySQL to Google Sheets."""
    last_db_hash = ""  # Initialize with an empty hash

    while not exit_flag:  # Exit if the flag is set to True
        current_db_data = fetch_from_mysql()
        new_db_hash = calculate_data_hash(current_db_data)

        # Only update Sheets if the DB has changed and Sheets is not the source of the latest change
        if new_db_hash != last_db_hash and new_db_hash != last_sheet_hash:
            ...
            if lock.acquire(blocking=False):
                try:
                    ...
                    last_db_hash = new_db_hash  # Update the last known DB hash
                finally:
...
def sheets_to_db_sync(last_db_hash):
    """Synchronize data from Google Sheets to MySQL."""
    last_sheet_hash = ""  # Initialize with an empty hash
    last_data = read_sheet_data()  # Initialize with the current state of the Sheets

    while not exit_flag:  # Exit if the flag is set to True
        new_data = read_sheet_data()  # Fetch the latest data from Google Sheets
        new_sheet_hash = calculate_data_hash(new_data)

        # Only update DB if the Sheets have changed and the DB is not the source of the latest change
        if new_sheet_hash != last_sheet_hash and new_sheet_hash != last_db_hash:
            # Only acquire the lock if a change is detected
            if lock.acquire(blocking=False):
                try:
                    ...
                    # Detect changes between last synced data and current data
                    ...
                    if rows_to_insert_or_update:
                        ...
                        insert_into_mysql(rows_to_insert_or_update)

                    if ids_to_delete:
                        print("Deleting rows in DB...")
                        delete_from_mysql(ids_to_delete)

                    # Update last_data and last_sheet_hash to the current state after sync
                    last_data = new_data
                    last_sheet_hash = new_sheet_hash
                    last_db_hash = calculate_data_hash(fetch_from_mysql())  # Refresh the DB hash after the sync
                finally:
...
if __name__ == "__main__":
    ...
    # Run both syncs concurrently with last known hash
    t1 = threading.Thread(target=db_to_sheets_sync, args=(last_sheet_hash,))
    t2 = threading.Thread(target=sheets_to_db_sync, args=(last_db_hash,))
